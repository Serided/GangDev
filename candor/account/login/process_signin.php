...
$stmt = $pdo->prepare("
    SELECT id, email, username, password_hash, email_verified::int AS email_verified
    FROM candor.users
    WHERE email = ?
    LIMIT 1
");
...
if (empty($user['email_verified'])) {
	...
	$upd = $pdo->prepare("
		UPDATE candor.users
		SET verify_token = ?, verify_expires_at = ?, last_verification_sent_at = NOW()
		WHERE id = ?
	");
...
';
	...
	$sent = sendMail($fromEmail, $fromName, $user['email'], $name, $subject, $htmlBody, $altBody);

	if (!$sent) {
		header('Location: /login/signin.php?error=' . urlencode('Could not send verification email. Try again.'));
		exit;
	}

...
(() => {
    ...
    function tickClock(){
        const d = new Date();
        $("clock").textContent = `LOCAL ${pad(d.getHours())}:${pad(d.getMinutes())}`;
    }
    ...
    // Simple animated demo values (landing-only)
    const state = { t:128, open:7, esc:1, audit:94, ex:3, ct:22, bar:68 };
    ...
    // Contact submit: keep it quiet and non-embarrassing
    window.DCOPS_submitContact = (e) => {
        e.preventDefault();
        const note = document.querySelector(".formNote");
        if (note) {
            note.textContent = "Received. We'll respond shortly.";
            note.style.color = "rgba(167,243,208,.95)";
            setTimeout(() => {
                note.textContent = "Contact intake is currently being finalized.";
                note.style.color = "";
            }, 3500);
        }
        return false;
